class Vendor::BaseController < ApplicationController
  rescue_from ActiveRecord::RecordNotFound, with: :record_not_found

  # solving the ActionController::InvalidAuthenticityToken error
  protect_from_forgery

  # 500 Error Helper
  if Rails.env.production?
    # Handling CanCan::AccessDenied
    rescue_from CanCan::AccessDenied, with: :rescue_access_denied

    # Exceptionを補足
    rescue_from Exception, with: :rescue_internal_server_error
  end

  before_action :authenticate_order_owner_account!
  before_action :set_layout_view_variables
  before_action :set_content_header
  before_action :set_paper_trail_whodunnit

  # Cancancan method
  load_and_authorize_resource

  layout 'vendor/layouts/application'

  # add flash message types
  add_flash_types :success, :info, :warning, :danger

  def current_user
    current_order_owner_account.order_owner
  end

  def current_ability
    @current_ability ||= OrderOwnerAbility.new(current_user)
  end

  private

  def user_for_paper_trail
    order_owner_account_signed_in? ? current_user.name : "Please Login in" # or whatever
  end

  def record_not_found(exception)
    Slack500.post(request, exception)

    render 'admin/404', status: 404
  end

  def rescue_internal_server_error(exception)
    # Slackに通知
    Slack500.post(request, exception)

    render 'admin/500', status: :internal_server_error, layout: 'application'
  end

  def rescue_access_denied(exception)
    Slack500.post(request, exception)

    redirect_to vendor_orders_path, danger: t(:'message.access_failed')
  end

  def set_layout_view_variables
    @layout_view_variables = {
      content_header: {
        header: '',
        subheader: { title: '', url: '' },
        labels: [],
        breadcrumbs: []
      }
    }
  end

  def set_content_header
    return if request.format == :json # json format dont need content header, so skip this implement checking

    err_message = "You haven't override 'set_content_header' method in '#{params[:controller]}' controller."
    if Rails.env.production?
      ExceptionNotifier.notify(err_message)
    else
      raise err_message
    end
  end
end
