class TransportExport::Report < ReportBase
  #############################################################################
  # Overriding function
  #############################################################################
  ...
  def on_run(params)
    ...
    when 'csv', 'tsv', 'xlsx'
      orders.each do |order|
        hash = {}
        TransportExport.export(order).each_with_index do |element, index|
          hash[header[index]] = element
        end
        result << hash
      end
      cols_data_type = TransportExport.cols_data_type
      cols_style = TransportExport.cols_style
    ...
    report_title = {
      layer1: {
        order_processor: {
          ...
          merge_cell_data: [I18n.t(:'reports.transport_export.processor_title'), "", "", "", "", "", ""],
          title_style: :header
        }
      },
      layer2: {
        order: {
          ...
          merge_cell_data: [I18n.t(:'reports.transport_export.order_title')]
        },
        order_product: {
          merge_cell_size: "B3:D4",
          merge_cell_data: [I18n.t(:'reports.transport_export.product_title'), "", ""]
        },
        order_shipment: {
          merge_cell_size: "E3:F4",
          merge_cell_data: [I18n.t(:'reports.transport_export.shipment_title'), ""]
        },
        order_check: {
          merge_cell_size: "G3:G4",
          merge_cell_data: [I18n.t(:'reports.transport_export.check_title')]
        }
      }
    }
    report_title
